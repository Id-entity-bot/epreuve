...
include 'connect_db.php';
  ...
  $address=$_POST['add'];
  if (!empty($oname) && !empty($username) && !empty($institute) && !empty($password_again) && !empty($password) && !empty($email) && !empty($phone_no) && !empty($address))
{ 
  if($password==$password_again){
    ...
    if(mysqli_num_rows($query1)==0){
		if(mysqli_num_rows($query2)==0){
			...
			$query=mysqli_query($conn,"INSERT INTO users (timeofreg, name, username, college, password, salt, email , phone, address,last_correct) values(NOW(),'$oname','$username', '$institute', '$hashed_pw','$salt', '$email', '$phone_no', '$address',NOW())");
			if($query){
